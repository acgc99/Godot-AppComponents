...
void ACHTTPImage::_make_request(){
    if(http_request->is_node_ready() && url != ""){
...
void ACHTTPImage::set_url(const String url){
    this->url = url;
    _make_request();
}


int ACHTTPImage::get_extension() const {
    return extension;
}


void ACHTTPImage::set_extension(const int extension){
